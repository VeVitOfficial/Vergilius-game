...
export class HUD {
    ...
    updatePrompt(nearBook, nearHome, hasBooks, bookName = '', nearDoor = null, nearExit = false) {
        if (nearExit) {
            this.showPrompt('[ E ] Opustit budovu');
        } else if (nearDoor) {
            this.showPrompt(`[ E ] Vstoupit: ${nearDoor.label}`);
        } else if (nearBook) {
